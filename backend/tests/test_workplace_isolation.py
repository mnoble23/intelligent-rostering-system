...
def test_availability_bulk_rejects_submission_when_user_cannot_reach_minimum_hours(monkeypatch):
    client, SessionLocal = _build_test_client()
    ...
    monkeypatch.setattr("app.api.availability.solver_is_available", lambda: True)
    monkeypatch.setattr("app.api.availability.calculate_max_feasible_minutes_for_user", lambda *args, **kwargs: 240)
...
def test_availability_bulk_reports_solver_unavailable_explicitly(monkeypatch):
    client, SessionLocal = _build_test_client()
    ...
    monkeypatch.setattr("app.api.availability.solver_is_available", lambda: False)
    ...
    detail = response.json()["detail"]
    assert detail["code"] == "solver_unavailable"
    assert detail["context"]["user_id"] == seeded["staff_a"].id
    assert detail["context"]["user_name"] == "staff_a"
